use crate::space::{DiscreteSpace, DiscreteSpaceArray, OneDimensional, TwoDimensional};
...
impl<const X: usize> ImageSpaceArrayRenderer<1> for OneDimensional<X> {
    fn render(&self) -> RgbImage {
        let state = DiscreteSpaceArray::read_state(self);
        let size = DiscreteSpaceArray::size(self);

        ImageBuffer::from_fn(size[0] as u32, 1, |x, _| {
